...
import llama_cpp
from llama_cpp import llama_set_adapters_lora, llama_adapter_lora_init, LlamaState
...
actived_adapter, current_actived_adapter_scale = active_adapter(slm.ctx, 'adapter_b', adapters_pointer_python_list, adapters_pointers_c_array, scales_c_float_array, personalized_scale=0.5)
print(f'adapter ativo: {actived_adapter}, scale: {current_actived_adapter_scale}')

kv_cache_adapters['adapter_b']['messages'].append(create_user_message('Qual a capital da Bahia?'))

test_inference_with_adapter = slm.create_chat_completion(messages=kv_cache_adapters['adapter_a']['messages'], max_tokens=100, stream=False)

print(f'teste da inferência com o {active_adapter}, com scale {current_actived_adapter_scale}, resultado:{test_inference_with_adapter}')

kv_cache_adapters['adapter_b']['state'] = slm.save_state()

print(kv_cache_adapters['adapter_b']['state'].n_tokens)
